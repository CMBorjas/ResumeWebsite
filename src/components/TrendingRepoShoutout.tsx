...
export default function TrendingRepoShoutout() {
    ...
    async function fetchTrendingRepo() {
      try {
        ...
        if (data.items && data.items.length > 0) {
          // Pick one from the top 10 trending
          const randomIndex = Math.floor(Math.random() * data.items.length);
          setRepo(data.items[randomIndex]);
        }
      } catch (error) {
        console.error("Error fetching trending repo:", error);
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="p-6 flex flex-col h-full bg-gradient-to-br hover:from-yellow-400/5 hover:to-transparent transition-colors duration-500 relative group overflow-hidden">
      ...
      <div className="absolute top-0 right-0 bg-yellow-400 text-black text-[10px] font-extrabold uppercase tracking-widest py-1 px-3 rounded-bl-xl shadow-lg z-10">
        Trending Now
      </div>

      <div className="flex items-center mb-4 mt-2">
        <div className="w-10 h-10 rounded-full flex items-center justify-center mr-3 group-hover:scale-110 transition-transform duration-300 border border-yellow-400/40 overflow-hidden bg-black/50 shadow-[0_0_10px_color-mix(in_srgb,var(--color-yellow-400)_40%,transparent)]">
      ...
      <div className="flex items-center justify-between mt-auto pt-4 border-t border-yellow-400/20 relative z-10">
